...
destroy () {
  ...
  echo ..
  echo start deleting...
  gcloud -q compute firewall-rules delete $1 --project ${GCP_PROJECT}
  echo finished.
}

gcloud compute firewall-rules list --project ${GCP_PROJECT} | awk '$1 != "NAME" {printf "__name__:%s,__network__:%s\n",$1,$2}' | while read line
do
